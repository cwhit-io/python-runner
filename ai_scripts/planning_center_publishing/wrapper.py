#!/usr/bin/env python3
"""
Planning Center Publishing API wrapper for AI agents.

Supports managing episodes, series, speakers, and episode resources
...
SERVICE_NAME = "planning_center_publishing"
...
def handle_action(data: dict, action: str) -> str:
    ...
    if action == "list_episodes":
        return planning_center_request("GET", f"{BASE_URL}/episodes", action, params=params)

    if action == "get_episode":
        require_fields(data, ["episode_id"], action)
        return planning_center_request(
            "GET", f"{BASE_URL}/episodes/{data['episode_id']}", action, params=params
        )
    ...
    if action == "get_series":
        require_fields(data, ["series_id"], action)
        return planning_center_request(
            "GET", f"{BASE_URL}/series/{data['series_id']}", action, params=params
        )

    if action == "list_speakers":
        return planning_center_request("GET", f"{BASE_URL}/speakers", action, params=params)

    if action == "list_episode_resources":
        require_fields(data, ["episode_id"], action)
        return planning_center_request(
            "GET", f"{BASE_URL}/episodes/{data['episode_id']}/resources",
    ...
    if action == "create_episode":
        return handle_write(data, action, "POST", f"{BASE_URL}/episodes")
