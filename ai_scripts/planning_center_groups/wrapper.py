#!/usr/bin/env python3
"""
Planning Center Groups API wrapper for AI agents.

Supports listing, searching, and managing groups, memberships,
...
SERVICE_NAME = "planning_center_groups"
...
def handle_action(data: dict, action: str) -> str:
    ...
    if action == "list_groups":
        return planning_center_request("GET", f"{BASE_URL}/groups", action, params=params)

    if action == "search_groups":
        query = data.get("query", "")
        if query:
            params["query"] = query
        return planning_center_request("GET", f"{BASE_URL}/groups/search", action, params=params)

    if action == "get_group":
        require_fields(data, ["group_id"], action)
        return planning_center_request(
            "GET", f"{BASE_URL}/groups/{data['group_id']}", action, params=params
        )

    if action == "list_group_memberships":
        require_fields(data, ["group_id"], action)
        return planning_center_request(
            "GET", f"{BASE_URL}/groups/{data['group_id']}/memberships", action, params=params
        )

    if action == "list_group_events":
        require_fields(data, ["group_id"], action)
        return planning_center_request(
            "GET", f"{BASE_URL}/groups/{data['group_id']}/events", action, params=params
        )

    if action == "list_group_types":
        return planning_center_request("GET", f"{BASE_URL}/group_types", action, params=params)
    ...
    if action == "create_group_event":
        require_fields(data, ["group_id", "data"], action)
        url = f"{BASE_URL}/groups/{data['group_id']}/events"
        if data.get("dry_run", False):
            return build_dry_run_response(action, "POST", url, body=data["data"])
        return planning_center_request("POST", url, action, json_body=data["data"])
