#!/usr/bin/env python3
"""
Planning Center Check-Ins API wrapper for AI agents.

Supports listing events, check-ins, locations, event periods,
and attendees via the Planning Center Check-Ins API.
...
SERVICE_NAME = "planning_center_checkins"
...
def handle_action(data: dict, action: str) -> str:
    ...
    if action == "list_check_ins":
        params = build_params(data)
        if data.get("event_id"):
            return planning_center_request(
                "GET", f"{BASE_URL}/events/{data['event_id']}/check_ins", action, params=params
            )
        return planning_center_request("GET", f"{BASE_URL}/check_ins", action, params=params)

    if action == "get_check_in":
        require_fields(data, ["check_in_id"], action)
        return planning_center_request(
            "GET", f"{BASE_URL}/check_ins/{data['check_in_id']}", action, params=params
        )

    if action == "list_locations":
        return planning_center_request("GET", f"{BASE_URL}/locations", action, params=params)

    if action == "list_event_periods":
        require_fields(data, ["event_id"], action)
        return planning_center_request(
            "GET", f"{BASE_URL}/events/{data['event_id']}/event_periods", action, params=params
        )
