#!/usr/bin/env python3
"""
Planning Center Services API wrapper for AI agents.

Supports managing service types, plans, plan items, plan people,
...
SERVICE_NAME = "planning_center_services"
...
def handle_action(data: dict, action: str) -> str:
    ...
    if action == "list_service_types":
        return planning_center_request("GET", f"{BASE_URL}/service_types", action, params=params)

    if action == "list_plans":
        # Optionally scope to a service type
        url = f"{BASE_URL}/plans"
        if data.get("service_type_id"):
            url = f"{BASE_URL}/service_types/{data['service_type_id']}/plans"
    ...
    if action == "get_plan":
        ...
        return planning_center_request(
            "GET", f"{BASE_URL}/plans/{data['plan_id']}", action, params=params
        )

    if action == "list_plan_items":
        require_fields(data, ["plan_id"], action)
        return planning_center_request(
            "GET", f"{BASE_URL}/plans/{data['plan_id']}/items", action, params=params
        )

    if action == "list_plan_people":
        require_fields(data, ["plan_id"], action)
        return planning_center_request(
            "GET", f"{BASE_URL}/plans/{data['plan_id']}/team_members", action, params=params
        )

    if action == "list_songs":
        return planning_center_request("GET", f"{BASE_URL}/songs", action, params=params)

    if action == "search_songs":
        query = data.get("query", "")
        if query:
            params["query"] = query
        return planning_center_request("GET", f"{BASE_URL}/songs/search", action, params=params)

    if action == "list_teams":
        return planning_center_request("GET", f"{BASE_URL}/teams", action, params=params)
    ...
    if action == "update_plan_title":
        require_fields(data, ["plan_id", "title"], action)
        body = {
            "data": {
                "type": "Plan",
                "id": data["plan_id"],
        ...
        if data.get("dry_run", False):
            return build_dry_run_response(action, "PATCH",
                                          f"{BASE_URL}/plans/{data['plan_id']}", body=body)
        return planning_center_request(
            "PATCH", f"{BASE_URL}/plans/{data['plan_id']}", action, json_body=body
        )

    if action == "create_plan_note":
        require_fields(data, ["plan_id", "content"], action)
        body = {
            "data": {
                "type": "Note",
        ...
        if data.get("category"):
            body["data"]["attributes"]["category"] = data["category"]
        if data.get("dry_run", False):
            return build_dry_run_response(action, "POST",
                                          f"{BASE_URL}/plans/{data['plan_id']}/notes", body=body)
        return planning_center_request(
            "POST", f"{BASE_URL}/plans/{data['plan_id']}/notes", action, json_body=body
        )
