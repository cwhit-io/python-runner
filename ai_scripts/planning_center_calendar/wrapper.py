#!/usr/bin/env python3
"""
Planning Center Calendar API wrapper for AI agents.

Supports listing, searching, and managing events, event instances,
...
SERVICE_NAME = "planning_center_calendar"
...
def handle_action(data: dict, action: str) -> str:
    ...
    if action == "search_events":
        ...
        if query:
            params["query"] = query
        return planning_center_request("GET", f"{BASE_URL}/events/search", action, params=params)
    ...
    if action == "list_rooms":
        return planning_center_request("GET", f"{BASE_URL}/rooms", action, params=params)

    if action == "list_tags":
        return planning_center_request("GET", f"{BASE_URL}/tags", action, params=params)

    # ── Write actions (guarded) ───────────────────────────────────────────

    if action == "create_event":
        return handle_write(data, action, "POST", f"{BASE_URL}/events")

    if action == "update_event":
        require_fields(data, ["event_id"], action)
        return handle_write(
            data, action, "PATCH", f"{BASE_URL}/events/{data['event_id']}"
        )
