#!/usr/bin/env python3
"""
Planning Center People API wrapper for AI agents.

Supports listing, searching, and managing people, households, campuses,
...
SERVICE_NAME = "planning_center_people"
...
def handle_action(data: dict, action: str) -> str:
    ...
    if action == "search_people":
        ...
        if query:
            params["query"] = query
        return planning_center_request("GET", f"{BASE_URL}/people/search", action, params=params)

    if action == "get_person":
        require_fields(data, ["person_id"], action)
        params = build_params(data)
        return planning_center_request(
            "GET", f"{BASE_URL}/people/{data['person_id']}", action, params=params
        )

    if action == "list_households":
        params = build_params(data)
        return planning_center_request("GET", f"{BASE_URL}/households", action, params=params)

    if action == "get_household":
        require_fields(data, ["household_id"], action)
        params = build_params(data)
        return planning_center_request(
            "GET", f"{BASE_URL}/households/{data['household_id']}", action, params=params
        )
    ...
    if action == "list_workflows":
        params = build_params(data)
        return planning_center_request("GET", f"{BASE_URL}/workflows", action, params=params)

    if action == "list_lists":
        params = build_params(data)
        return planning_center_request("GET", f"{BASE_URL}/lists", action, params=params)

    if action == "get_list":
        require_fields(data, ["list_id"], action)
        params = build_params(data)
        return planning_center_request(
            "GET", f"{BASE_URL}/lists/{data['list_id']}", action, params=params
        )
    ...
    if action == "create_person":
        return handle_write(data, action, "POST", f"{BASE_URL}/people")

    if action == "update_person":
        require_fields(data, ["person_id"], action)
        return handle_write(
            data, action, "PATCH", f"{BASE_URL}/people/{data['person_id']}"
        )

    return error_result(action, f"Unknown action: {action}")


def handle_write(data: dict, action: str, method: str, url: str) -> str:
    """Handle a guarded write action with dry-run support."""
    if data.get("dry_run", False):
        return build_dry_run_response(action, method, url, body=data.get("data", {}))

    require_fields(data, ["data"], action)
    return planning_center_request(method, url, action, json_body=data["data"])


def build_params(data: dict) -> dict:
    """Build query parameters from common optional fields."""
    params = {}
